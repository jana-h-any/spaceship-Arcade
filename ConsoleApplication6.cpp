...
void keyboard(unsigned char key, int x, int y) {
    if (currentState == MENU) {
        if (key == '1') {
            system("\"E:\\jana\\opengl projects\\ConsoleApplication3\\Debug\\ConsoleApplication3.exe\"");
        }
        else if (key == '2') {
            system("\"E:\\jana\\opengl projects\\ConsoleApplication5\\Debug\\ConsoleApplication5.exe\"");
        }
    }
    else if (key == 27) {
        currentState = MENU;
